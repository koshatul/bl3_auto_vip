DOCKER_REPO := matt1484/bl3_auto_vip
...
DOCKER_BUILD_REQ += artifacts/build/release/linux/amd64/bl3-auto-vip
...
.PHONY: install
install: artifacts/build/release/$(GOHOSTOS)/$(GOHOSTARCH)/bl3-auto-vip
	install "$(<)" /usr/local/bin/

.PHONY: run
run: artifacts/build/debug/$(GOHOSTOS)/$(GOHOSTARCH)/bl3-auto-vip
	$< $(RUN_ARGS)

.PHONY: docker-run
docker-run: docker
	-@mkdir -p artifacts/config
	docker run -ti --rm -v "$(shell pwd)/artifacts/config:/root/.config/bl3-auto-vip" $(DOCKER_REPO):$(DOCKER_TAGS) 
